...
export default async function TarifsPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-slate-50 text-slate-900">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-[420px] -z-10 h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent"
      />
        ...
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3 md:px-6">
          ...
          <nav className="flex items-center gap-2">
            {user ? (
              <Link
                href="/categories"
                className="inline-flex h-9 items-center justify-center gap-1.5 rounded-md bg-slate-900 px-4 text-sm font-medium text-white transition hover:bg-slate-800"
              >
                Tableau de bord
              ...
              <>
                <Link
                  href="/login"
                  className="inline-flex h-9 items-center justify-center rounded-md px-4 text-sm font-medium text-slate-700 transition hover:bg-slate-100"
                >
                  Connexion
                </Link>
                <Link
      ...
      <main className="relative">
        <section className="mx-auto w-full max-w-6xl px-4 pb-10 pt-14 text-center md:px-6 md:pt-20">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-blue-200 bg-white/70 px-3 py-1 text-xs font-medium text-blue-700 shadow-ride-xs backdrop-blur">
            <Cloud className="h-3.5 w-3.5" aria-hidden />
            Tarifs simples, sans engagement
          </div>
          <h1 className="mt-5 text-balance text-4xl font-semibold tracking-tight text-slate-900 md:text-5xl lg:text-6xl">
            Un plan pour chaque garage.
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-balance text-base text-slate-600 md:text-lg">
        ...
        <section className="mx-auto w-full max-w-6xl px-4 pb-20 md:px-6">
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              <article
                key={title}
                className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-ride-sm"
              >
                <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-blue-200 bg-blue-50 text-blue-700 shadow-ride-xs">
        ...
        <section className="mx-auto w-full max-w-3xl px-4 pb-24 md:px-6">
          ...
          <div className="mt-8 space-y-3">
            {[
              ...
              <details
                key={q}
                className="group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-ride-sm transition hover:border-slate-300"
              >
                <summary className="flex cursor-pointer items-start justify-between gap-4 text-sm font-medium text-slate-900">
                  <span>{q}</span>
                  <span
                    aria-hidden
                    className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-500 transition group-open:rotate-45"
                  >
                    +
                  </span>
                </summary>
      ...
      <footer className="border-t border-slate-200/70 bg-white/60 backdrop-blur-sm">
        <div className="mx-auto flex w-full max-w-6xl flex-col items-center justify-between gap-3 px-4 py-6 text-xs text-slate-500 sm:flex-row md:px-6">
          ...
          <nav className="flex flex-wrap items-center gap-x-4 gap-y-1">
            <Link href="/mentions-legales" className="transition hover:text-slate-900">
              Mentions légales
            </Link>
            <Link href="/cgu" className="transition hover:text-slate-900">
              CGU
            </Link>
            <Link href="/confidentialite" className="transition hover:text-slate-900">
              Confidentialité
            </Link>
            <Link href="/rgpd" className="transition hover:text-slate-900">
              RGPD
            </Link>
          </nav>
        </div>
